...
class Interpreter(private var context: Context) {
	fun interpret(node: Node): Any? = when (node) {
		is StatementNode -> interpret(node.node)
		is ParenthesizedOperationNode -> interpret(node.node)
		
		is StringNode -> node.string
		is NumberNode -> node.value
		
		is IdentifierNode -> context.findVariable(node.name)?.getValue()
				...
				"--", "++" -> {
					if (node.operand is IdentifierNode) {
						...
						node.isPrefixed.not().let {
							if (node.operator == "++") variable.increment(it)
							else /*node.operator == "--"*/ variable.decrement(it)
						}
				...
				"=" -> {
					when (node.lhs) {
						is IdentifierNode -> {
							...
							value.also { variable.setValue(it) }
						}
						...
						is MemberAccessNode -> {
							val variable = context.findVariable(node.lhs.name)
								?: throw InterpreterException("no variable named ${node.lhs.name}") at node.lhs.range
							val value = interpret(node.rhs)
							value.also { variable.setProperty(node.lhs.member, it) }
						}
		...
		is FunctionDeclarationNode -> {
			...
			if (context.addVariable(function)) function
			else throw InterpreterException("function ${node.name} already exists")
		}
		
		is FunctionCallNode -> {
			context.findVariable(node.name)?.invoke(
		...
		is MemberAccessNode -> {
			context.findVariable(node.name)?.getProperty(node.member)
				?: throw InterpreterException("No variable named ${node.name}") at node.range
		}
