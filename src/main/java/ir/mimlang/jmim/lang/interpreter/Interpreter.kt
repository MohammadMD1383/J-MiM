...
class Interpreter(private var context: Context) {
    fun interpret(node: Node): Any? {
        when (node) {
            ...
            is StringNode -> {
                if (node.isRaw) return node.string else {
                    val result = Regex("\\$[a-zA-Z-0-9-_]+").findAll(node.string).toList()
                    var text = ""
                    return if (result.isNotEmpty()) {
                        for (item: Int in 0..result.count() - 1) {
                            text = node.string.replace(result[item].value, context.findVariable(result[item].value.removeRange(0, 1))!!.getValue().toString())
                        }
                        text
                    } else {
                        node.string
                ...
                val andResult = andGroup.cases.let andResult@{ cases ->
                    cases.forEach { case ->
                        val caseResult = interpret(
                                BinaryOperationNode(
                                        operand,
                                        case.operator ?: defaultComparator
                                        ?: throw InterpreterException("no operator found to check case condition") at case.range,
                                        case.operand,
                                        case.range
                                )
                        ) as Boolean
